...
function createSparkle(x, y) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const color = Math.random() > 0.45 ? '#f5f2ee' : '#a8842c';
...
glowOrb.style.cssText = [
  ...
  'background:radial-gradient(ellipse at 40% 50%, rgba(168,132,44,0.22) 0%, rgba(168,132,44,0.10) 40%, transparent 70%)',
  'filter:blur(28px)',
  ...
  blocks.forEach(block => {
    ...
    steps.forEach((step, i) => {
      ...
      tl.to(step.querySelector('.wf-node'), {
        borderColor: 'rgba(168,132,44,.85)',
        background:  'rgba(168,132,44,.2)',
        boxShadow:   '0 0 16px rgba(168,132,44,.6), 0 0 32px rgba(168,132,44,.25)',
...
(function () {
  ...
  function tick() {
    ...
    card.style.boxShadow = `
      ${-currentRY * 1.2}px ${currentRX * 1.2}px 40px rgba(168,132,44,.55),
      0 0 80px rgba(168,132,44,.2)
    `;
...
(function () {
  ...
  tl.to(autoTrack, { x: 0, ease: 'none', duration: 1 })
    .to(autoTrack, { x: '-80%', ease: 'none', duration: 9 });
...
// Sound buttons (Slide 0: AI Job Finder Pipeline, Slide 2: AI Video Studio) — desktop + mobile
(function () {
  function addSoundButton(slideIndex) {
