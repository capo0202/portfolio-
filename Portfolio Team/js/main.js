...
function createSparkle(x, y) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const color = Math.random() > 0.45 ? '#f5f2ee' : '#6366f1';
...
glowOrb.style.cssText = [
  ...
  'background:radial-gradient(ellipse at 40% 50%, rgba(99,102,241,0.22) 0%, rgba(99,102,241,0.10) 40%, transparent 70%)',
  'filter:blur(28px)',
  ...
  blocks.forEach(block => {
    ...
    steps.forEach((step, i) => {
      ...
      tl.to(step.querySelector('.wf-node'), {
        ...
        boxShadow:   '0 0 16px rgba(99,102,241,.6), 0 0 32px rgba(99,102,241,.25)',
...
(function () {
  ...
  function tick() {
    ...
    card.style.boxShadow = `
      ${-currentRY * 1.2}px ${currentRX * 1.2}px 40px rgba(99,102,241,.55),
