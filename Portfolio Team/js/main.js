...
function createSparkle(x, y) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const color = Math.random() > 0.45 ? '#f5f2ee' : '#2dd4a8';
...
glowOrb.style.cssText = [
  ...
  'background:radial-gradient(ellipse at 40% 50%, rgba(45,212,168,0.22) 0%, rgba(45,212,168,0.10) 40%, transparent 70%)',
  'filter:blur(28px)',
  ...
  blocks.forEach(block => {
    ...
    steps.forEach((step, i) => {
      ...
      tl.to(step.querySelector('.wf-node'), {
        borderColor: 'rgba(45,212,168,.85)',
        background:  'rgba(45,212,168,.2)',
        boxShadow:   '0 0 16px rgba(45,212,168,.6), 0 0 32px rgba(45,212,168,.25)',
...
(function () {
  ...
  function tick() {
    ...
    card.style.boxShadow = `
      ${-currentRY * 1.2}px ${currentRX * 1.2}px 40px rgba(45,212,168,.55),
      0 0 80px rgba(45,212,168,.2)
    `;
